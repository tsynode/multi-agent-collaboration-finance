...
DEFAULT_AGENT_IAM_ROLE = "DEFAULT_AgentExecutionRole"
...
class AgentsForAmazonBedrock:
    ...
    def __init__(self):
        ...
        self._bedrock_agent_runtime_client = boto3.client('bedrock-agent-runtime', 
                                                          config=long_invoke_time_config)
    ...
    def _create_lambda_iam_role(self, agent_name: str,
                                ...
                                enable_trace: bool=False) -> object:
        ...
        if additional_function_iam_policy is not None:
            if enable_trace:
                print(f"Attaching additional IAM policy to Lambda role:\n{additional_function_iam_policy}")
    ...
    def create_lambda(self, 
                      ...
                      additional_function_iam_policy: Dict=None,
                      sub_agent_arns: List[str]=None) -> str:
        ...
        _lambda_function = self._lambda_client.create_function(
            ...
            Role=self._create_lambda_iam_role(agent_name, additional_function_iam_policy, sub_agent_arns),
    ...
    def _create_agent_role(self, 
        ...
        if verbose:
            print(f"Role {_agent_role_name} created. ARN: {_agent_role['Role']['Arn']}")
            print(f"Adding bedrock_allow_policy to role {_agent_role_name}\n{_bedrock_policy_json}...")
    ...
    def wait_agent_alias_status_update(self, agent_id, agent_alias_id, verbose=False):
        response = self._bedrock_agent_client.get_agent_alias(agentId=agent_id,
                                                              agentAliasId=agent_alias_id)
        agent_alias_status = response['agentAlias']['agentAliasStatus']
        while agent_alias_status.endswith('ING'):
            if verbose:
                print(f'Waiting for agent ALIAS status to change. Current status {agent_alias_status}')
            time.sleep(5)
            try:
                response = self._bedrock_agent_client.get_agent_alias(agentId=agent_id,
                                                              agentAliasId=agent_alias_id)
                ...
                agent_status = 'DELETED'
        if verbose:
            print(f'Agent id {agent_id}, Alias {agent_alias_id} current status: {agent_alias_status}')

    def associate_sub_agents(self,
        supervisor_agent_id, 
        sub_agents_list
    ):
        for sub_agent in sub_agents_list:
            ...
            self.wait_agent_status_update(supervisor_agent_id)
            self._bedrock_agent_client.prepare_agent(
                agentId=supervisor_agent_id
            )
            self.wait_agent_status_update(supervisor_agent_id)
    ...
    def create_agent(self,
        ...
        _agent_id = response['agent']['agentId']
        ...
        self.wait_agent_status_update(_agent_id)
        _agent_alias = self._bedrock_agent_client.create_agent_alias(
            agentAliasName='v1', agentId=_agent_id
        )
        _agent_alias_id = _agent_alias['agentAlias']['agentAliasId']
        _agent_alias_arn = _agent_alias['agentAlias']['agentAliasArn']
    ...
    def add_action_group_with_lambda(self,
        ...
        # self.wait_agent_status_update(_agent_id)
        # _agent_alias = self._bedrock_agent_client.create_agent_alias(
        #     agentAliasName='v2', agentId=_agent_id
        # )
        # _agent_alias_id = _agent_alias['agentAlias']['agentAliasId']
        # _agent_alias_arn = _agent_alias['agentAlias']['agentAliasArn']
        ...
        # self.wait_agent_status_update(_agent_id)
        # time.sleep(5) # make sure agent is ready to be invoked as soon as we return
        return #_agent_alias_id, _agent_alias_arn
    ...
    def create_supervisor_agent(self, supervisor_agent_name: str, 
        ...
        if kb_arn is not None:
            ...
            _supervisor_role_arn = self._create_agent_role(supervisor_agent_name, model_ids)
    ...
    def _make_fully_cited_answer(self, orig_agent_answer, event, enable_trace=False, trace_level="none"):
        _citations = event.get('chunk', {}).get('attribution', {}).get('citations', [])
        if _citations:
            if enable_trace:
                print(f"got {len(event['chunk']['attribution']['citations'])} citations \n")
        else:
    ...
    def invoke(self,
        ...
        if enable_trace:
            if trace_level == "all":
                ...
                print(f"invokeAgent API request ID: {_agent_resp['ResponseMetadata']['RequestId']}")
                    ...
                    if 'orchestrationTrace' in _event['trace']['trace']:
                            ...
                            if "invocationInput" in _orch:
                                ...
                                if 'actionGroupInvocationInput' in _input:
                                    ...
                                    if trace_level == "outline":
                                        ...
                                        print(colored(f"{_collab_input_text}\n", "magenta"))
                            ...
                            if "observation" in _orch:
                                if trace_level == "core":
                                    ...
                                    if 'finalResponse' in _output:
                                        print(colored(f"Final response:\n{_output['finalResponse']['text']}", "cyan"))
                        ...
                        if 'modelInvocationInput' in _orch:
                            if _sub_agent_alias_id is not None:
                                _sub_step += 1
                                print(colored(f"\n---- Step {_orch_step}.{_sub_step} [using sub-agent name:{_sub_agent_name}, id:{_sub_agent_alias_id}] ----", "green"))
                            else:
                                _orch_step += 1
                                _sub_step = 0
                                print(colored(f"\n---- Step {_orch_step} ----", "green"))
